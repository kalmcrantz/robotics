...
int main(int argc, char** argv) {
    ...
    bool go_on = false;
	int j = 0;
	while (j < nparts_misplaced) {
        model_to_part(misplaced_models_actual_coords_wrt_world[j], current_part, box_location_code);
        //adjust_part_location_no_release(Part sourcePart, Part destinationPart, double timeout = MAX_ACTION_SERVER_WAIT_TIME);
        int index_des_part = part_indices_misplaced[j];
	...
	j = 0;
	while (j < part_indices_missing.size()) {
    //populate missing parts  in box:
    // should repeat for ALL  missing parts
        int n_missing_part = part_indices_missing[j];
