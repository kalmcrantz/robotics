...
int main(int argc, char** argv) {
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
		for (int i=0;i<nparts;i++) {
			geometry_msgs::Pose pose = orphan_models_wrt_world[i].pose;
			//find the orphaned part whose position is within the error of the current part (bad part)
			if (abs(pose.position.x - current_part.pose.pose.position.x) < ERROR && abs(pose.position.y - 	current_part.pose.pose.position.y) < ERROR && abs(pose.position.z - current_part.pose.pose.position.z) < ERROR) {
